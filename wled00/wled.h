...
#define WLED_CODENAME "Lightdrive V1.0"
...
WLED_GLOBAL char apSSID[33] _INIT("Astral Vega");                             // AP off by default (unless setup)
...
// LED CONFIG
WLED_GLOBAL uint16_t ledCount _INIT(39);          // overcurrent prevented by ABL
...
WLED_GLOBAL char serverDescription[33] _INIT("Astral Vega");  // Name of module
...
WLED_GLOBAL bool notifyMacro  _INIT(true);                       // send notification for macro
WLED_GLOBAL bool notifyHue    _INIT(true);                        // send notification if Hue light changes
WLED_GLOBAL bool notifyTwice  _INIT(true);                       // notifications use UDP: enable if devices don't sync reliably

WLED_GLOBAL bool alexaEnabled _INIT(true);                       // enable device discovery by Amazon Echo
WLED_GLOBAL char alexaInvocationName[33] _INIT("Vega");          // speech control name of device. Choose something voice-to-text can understand
...
// transitions
WLED_GLOBAL bool transitionActive _INIT(true);
...
WLED_GLOBAL byte effectIntensity _INIT(210);
WLED_GLOBAL byte effectFFT1 _INIT(6);
WLED_GLOBAL byte effectFFT2 _INIT(215);
WLED_GLOBAL byte effectFFT3 _INIT(252);
WLED_GLOBAL byte effectPalette _INIT(1);  //default pallete, set to rainbow by isaac


// Automatic Effect Changing Variables
WLED_GLOBAL bool AutoAudioEffectChanger _INIT(false);
WLED_GLOBAL int changeDelayMs _INIT(20000);  //time between audio changes
WLED_GLOBAL long lastChangeTime _INIT(0);  //Value to store the time since the last change 
WLED_GLOBAL byte nextDisplayedEffect _INIT(112);  //value to store the last used effect
...
WLED_GLOBAL uint16_t presetCycleTime _INIT(300);
